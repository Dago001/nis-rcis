/**
 * Nigeria States and Local Government Areas (LGAs) Data & Auto-population Helper
 * Nigeria Immigration Service - Residence Card Issuance System (NIS-RCIS)
 */
...
const NIGERIA_STATES_LGAS = {
    ...
    "Ogun": [
        ...
        "Ikenne", "Ilugun", "Imeko Afon", "Ipokia", "Obafemi Owode", "Odeda", "Odogbolu",
    ...
    "Yobe": [
        ...
        "Karasuwa", "Machina", "Nangere", "Potiskum", "Tarmuwa", "Yunusari", "Yusufari"
    ],
    "Zamfara": [
...
/**
 * Populate LGAs dropdown based on selected State
 * @param {string} state - Selected state name
 * @param {string} lgaSelectId - DOM ID of the LGA <select> element
 * @param {string} preselectedLga - Optional LGA to mark selected
 */
function populateLgas(state, lgaSelectId, preselectedLga) {
    ...
    if (!state || !NIGERIA_STATES_LGAS[state]) {
        ...
        defaultOpt.value = '';
        defaultOpt.textContent = '-- SELECT LGA (SELECT STATE FIRST) --';
    ...
    lgas.forEach(lga => {
        ...
        opt.value = lga;
        opt.textContent = lga;
        if (preselectedLga && (preselectedLga.toUpperCase() === lga.toUpperCase() || preselectedLga === lga)) {
            opt.selected = true;
        }
